...
class CopyTradingController extends Controller
{
    ...
    {
        $user=Auth::user();
        $strategies=CopyStrategy::with(['profile.user.stockHoldings'])
            ->withCount(['relationships as copier_count'=>fn($q)=>$q->where('status','active')])
            ->where('is_public',true)->where('is_active',true)
            ->whereHas('profile',fn($q)=>$q->whereNotNull('approved_at')->where('is_public',true)->where('is_accepting_copiers',true))
    ...
    public function myCopies(TradingPerformanceService $performance)
    {
        $relationships=CopyRelationship::with(['strategy.profile.user','provider'])
            ->where('follower_id',Auth::id())->latest()->get();
        foreach ($relationships as $relationship) { $relationship->performance_metrics = $performance->copyRelationship($relationship); }
    ...
    public function apply(Request $request)
    {
        ...
        if($user->copyTraderProfile?->approved_at){
            return redirect()->route('copy-trading.provider.dashboard')->with('success','You are already an approved strategy provider.');
        }
    ...
    public function providerDashboard()
    {
        ...
        $executions=\App\Models\CopyTradeExecution::with(['relationship.follower','providerTrade.stock'])
    ...
    public function follow(Request $request, CopyStrategy $strategy)
    {
        ...
        abort_unless($strategy->is_public && $strategy->is_active && $strategy->profile->approved_at && $strategy->profile->is_accepting_copiers,404);
        ...
        CopyRelationship::updateOrCreate(
            ['follower_id'=>$user->id,'provider_id'=>$strategy->profile->user_id],
            $data+['copy_strategy_id'=>$strategy->id,'status'=>'active','started_at'=>now(),'stopped_at'=>null]
        );

        return redirect()->route('copy-trading.my-copies')->with('success','Copy trading activated for '.$strategy->name.'.');
    }
    ...
    {
        $execution->load(['relationship.strategy.profile.user','relationship.provider','followerTrade.stock','providerTrade.stock']);
        ...
        return view('copy-trading.execution-show', compact('execution','currentPrice','profitLoss','returnPercent'));
    }

    public function status(Request $request, CopyRelationship $relationship)
    {
        abort_unless($relationship->follower_id===Auth::id(),403);
        $data=$request->validate(['status'=>'required|in:active,paused,stopped']);
        $relationship->update(['status'=>$data['status'],'stopped_at'=>$data['status']==='stopped'?now():null]);
        return back()->with('success','Copy relationship updated.');
    }

    public function executions()
    {
        $items=\App\Models\CopyTradeExecution::with(['relationship.strategy.profile.user','providerTrade.stock','followerTrade'])
