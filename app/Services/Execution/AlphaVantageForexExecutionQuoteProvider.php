...
final class AlphaVantageForexExecutionQuoteProvider implements ForexExecutionQuoteProvider
{
    ...
    {
        return 'alpha_vantage_realtime_fx';
    }
    ...
    public function quote(ForexPair $pair): array
    {
        ...
        $response = Http::timeout(12)
            ...
            ->get($this->baseUrl, [
                ...
                'from_currency' => strtoupper($pair->base_currency),
                'to_currency' => strtoupper($pair->quote_currency),
                'apikey' => $this->apiKey,
            ]);

        if (! $response->successful()) {
            throw new RuntimeException('Live Forex execution quote request failed.');
        }

        $data = $response->json();
        $message = $data['Note'] ?? $data['Information'] ?? $data['Error Message'] ?? null;
        if ($message) {
            throw new RuntimeException('Live Forex execution quote provider is temporarily unavailable.');
        }

        $quote = $data['Realtime Currency Exchange Rate'] ?? null;
        if (! is_array($quote)) {
            throw new RuntimeException('Live Forex execution quote payload is unavailable.');
        }
        ...
        if ($rate <= 0 || $lastRefreshed === '') {
            throw new RuntimeException('Live Forex execution quote is incomplete.');
        }

        try {
            $capturedAt = CarbonImmutable::parse($lastRefreshed, $timezone)->utc();
        } catch (\Throwable) {
            throw new RuntimeException('Live Forex execution quote timestamp is invalid.');
        }
