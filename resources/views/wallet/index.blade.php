<x-user-layout>
    <x-slot name="header">{{ localize('ui.money.money', 'Money') }}</x-slot>

    <div class="ui-page max-w-7xl space-y-5 sm:space-y-6">
        <section class="wallet-shell-card">
            <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="min-w-0">
                    <p class="text-sm font-medium text-muted-foreground">{{ financial_term('available_balance') }}</p>
                    ...
                    <div class="mt-3 flex items-center gap-2 text-xs text-muted-foreground">
                        ...
                        {{ localize('ui.money.account_ready', 'Money account ready') }}
                    </div>
                    <div class="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                        <span>{{ financial_term('balance') }} <strong class="font-medium text-foreground">{{ format_currency($wallet->balance) }}</strong></span>
                        <span>{{ financial_term('reserved_balance') }} <strong class="font-medium text-foreground">{{ format_currency($wallet->reserved_balance) }}</strong></span>
                    </div>
                </div>
        ...
        <section class="wallet-metrics-grid">
            <div class="wallet-metric-card">
                ...
                <div class="min-w-0">
                    <span>{{ localize('ui.money.total_deposits', 'Total deposits') }}</span>
                    <strong>{{ format_currency($totalDeposits) }}</strong>
                    <small>{{ localize('ui.money.completed_funding', 'Completed funding') }}</small>
                </div>
            </div>
            <div class="wallet-metric-card">
                <div class="wallet-metric-icon"><i data-lucide="arrow-up-from-line" class="h-4 w-4"></i></div>
                <div class="min-w-0">
                    <span>{{ localize('ui.money.total_withdrawals', 'Total withdrawals') }}</span>
                    <strong>{{ format_currency($totalWithdrawals) }}</strong>
                    <small>{{ localize('ui.money.completed_settlements', 'Completed settlements') }}</small>
                </div>
            </div>
            <div class="wallet-metric-card">
                <div class="wallet-metric-icon"><i data-lucide="chart-no-axes-combined" class="h-4 w-4"></i></div>
                <div class="min-w-0">
                    <span>{{ localize('ui.money.total_invested', 'Total invested') }}</span>
                    <strong>{{ format_currency($totalInvestments) }}</strong>
                    <small>{{ localize('ui.money.capital_allocated', 'Capital currently allocated') }}</small>
                </div>
            </div>
        </section>
        ...
        <section class="ui-panel overflow-hidden shadow-none">
            <div class="flex items-center justify-between gap-4 px-4 py-4 sm:px-5">
                <div>
                    <h2 class="text-base font-semibold tracking-tight text-foreground">{{ localize('ui.money.recent_activity', 'Recent activity') }}</h2>
                    <p class="mt-0.5 text-xs text-muted-foreground">{{ localize('ui.money.latest_activity', 'Your latest money activity.') }}</p>
                </div>
                <a href="{{ route('money.activity') }}" class="ui-button-secondary !h-9 !px-3 !py-0 text-xs">
                    {{ localize('ui.common.view_all', 'View all') }}
                ...
                @forelse($recentTransactions as $transaction)
                    ...
                    <div class="wallet-activity-row">
                        ...
                        <div class="shrink-0 text-right">
                            ...
                            <span class="wallet-status-badge wallet-status-{{ strtolower($transaction->status) }}">{{ localize_status($transaction->status) }}</span>
                        </div>
                    </div>
                @empty
                    <div class="m-4 app-empty-state">
                        <i data-lucide="wallet-cards" class="h-6 w-6"></i>
                        <p>{{ localize('ui.money.no_activity', 'No money activity yet.') }}</p>
                    </div>
                @endforelse
            </div>
        </section>
