<x-user-layout>
<x-slot name="header">{{ $categoryTitle ?? 'Investments' }}</x-slot>
...
<div class="mx-auto max-w-[1280px] space-y-6 px-3 py-5 sm:px-5 lg:px-6">
        ...
        <div class="grid gap-6 p-6 lg:grid-cols-[1.2fr_.8fr] lg:p-8">
            <div>
                <div class="flex items-center gap-2">
                    ...
                    <p class="text-[10px] font-semibold uppercase tracking-[.18em] text-red-600">Rcentz private investment market</p>
                </div>
                <h1 class="mt-3 max-w-3xl text-3xl font-semibold tracking-tight text-zinc-950 dark:text-white sm:text-4xl">
                    {{ $categoryTitle ?? 'One investment market. Multiple asset classes.' }}
                </h1>
                <p class="mt-3 max-w-2xl text-sm leading-6 text-zinc-600 dark:text-zinc-400">
                    Every listing below uses our system-authoritative investment price history. Public stock or crypto prices may inform future valuation inputs, but they never become the customer-facing investment unit price automatically.
                </p>
                <div class="mt-5 flex flex-wrap gap-2">
                    <a href="{{ route('account.investments') }}" class="rounded-xl bg-red-600 px-4 py-2 text-xs font-semibold text-white hover:bg-red-700">My Investments</a>
                    <a href="{{ route('account.investments.portfolio') }}" class="rounded-xl border border-zinc-200 px-4 py-2 text-xs font-semibold text-zinc-700 hover:bg-zinc-50 dark:border-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-900">Portfolio</a>
                </div>
            </div>
            <div class="grid grid-cols-2 gap-3">
                @foreach([
                    ['Instruments',$overview['instruments'] ?? \App\Models\PrivateInvestmentInstrument::where('is_visible',true)->count()],
                    ['Asset classes',$overview['categories'] ?? \App\Models\PrivateInvestmentInstrument::where('is_visible',true)->distinct('category')->count('category')],
                    ['Featured',$overview['featured'] ?? \App\Models\PrivateInvestmentInstrument::where('is_featured',true)->count()],
                    ['Underlying',currency_symbol().number_format($overview['underlying_valuation'] ?? \App\Models\PrivateInvestmentAsset::where('status','active')->sum('current_valuation'),0)],
                ] as [$label,$value])
        ...
        <section class="overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-950 shadow-sm dark:border-zinc-800"
            ...
            <div class="flex items-center justify-between border-b border-white/10 px-4 py-2.5">
                <div class="flex items-center gap-2">
                    ...
                    <p class="text-[9px] font-semibold uppercase tracking-[.16em] text-zinc-400">Investment listings</p>
                </div>
                <p class="text-[9px] text-zinc-500">Private Investment Price Authority</p>
            </div>
        ...
        @foreach($categoryCards as [$label,$routeName,$icon,$copy,$categoryKey])
            <a href="{{ route($routeName) }}" class="group rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-red-200 dark:border-zinc-800 dark:bg-zinc-950">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-[10px] font-semibold text-zinc-400">{{ $categoryStats[$categoryKey] ?? \App\Models\PrivateInvestmentInstrument::where('category',$categoryKey)->where('is_visible',true)->count() }} products</span>
                </div>
    ...
    @if(!empty($featured) && $featured->isNotEmpty())
        <section>
            <div class="mb-3 flex items-end justify-between"><div><p class="text-[10px] font-semibold uppercase tracking-[.14em] text-red-600">Featured</p><h2 class="mt-1 text-xl font-semibold">Featured opportunities</h2></div></div>
                ...
                @foreach($featured as $instrument)
                    ...
                    <a href="{{ route('investments.show',$instrument->slug) }}" class="group overflow-hidden rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm transition hover:border-red-200 dark:border-zinc-800 dark:bg-zinc-950">
                        ...
                        <div class="mt-4 flex flex-wrap gap-2 text-[9px]">
                            ...
                            <span class="rounded-full bg-zinc-100 px-2 py-1 dark:bg-zinc-900">Profit {{ currency_symbol() }}{{ number_format($featuredStory['net_term_min_profit'],0) }}–{{ currency_symbol() }}{{ number_format($featuredStory['net_term_max_profit'],0) }}</span>
                        </div>
                    </a>
                @endforeach
            </div>
    ...
    <div class="grid gap-5 xl:grid-cols-[1fr_320px]">
        <section>
            <form method="GET" action="{{ url()->current() }}" class="mb-4 flex flex-col gap-3 rounded-2xl border border-zinc-200 bg-white p-3 shadow-sm dark:border-zinc-800 dark:bg-zinc-950 md:flex-row md:items-center">
                <input name="search" value="{{ request('search') }}" placeholder="Search investments..." class="ui-input flex-1">
                @if(!isset($categoryTitle))
                    <select name="category" class="ui-input md:max-w-[220px]"><option value="">All asset classes</option>@foreach($categories as $category)<option value="{{ $category }}" @selected(request('category')===$category)>{{ ucwords(str_replace('_',' ',$category)) }}</option>@endforeach</select>
                @endif
                <select name="risk_level" class="ui-input md:max-w-[180px]"><option value="">All risk levels</option>@foreach(['low','medium','high','very_high'] as $risk)<option value="{{ $risk }}" @selected(request('risk_level')===$risk)>{{ ucwords(str_replace('_',' ',$risk)) }}</option>@endforeach</select>
                <button class="rounded-xl bg-red-600 px-5 py-2.5 text-xs font-semibold text-white hover:bg-red-700">Filter</button>
            </form>
                ...
                @forelse($instruments as $instrument)
                    ...
                    <a href="{{ route('investments.show',$instrument->slug) }}" class="group rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-red-200 hover:shadow-md dark:border-zinc-800 dark:bg-zinc-950">
                        ...
                        <div class="mt-4 grid grid-cols-2 border-y border-zinc-100 dark:border-zinc-900">
                            <div class="border-b border-r border-zinc-100 py-3 pr-3 dark:border-zinc-900"><p class="text-[9px] uppercase tracking-[.08em] text-zinc-400">Duration</p><p class="mt-1 text-xs font-semibold">{{ $listingStory['duration_label'] }}</p></div>
                            <div class="border-b border-zinc-100 py-3 pl-3 dark:border-zinc-900"><p class="text-[9px] uppercase tracking-[.08em] text-zinc-400">Return cycle</p><p class="mt-1 text-xs font-semibold">{{ $listingStory['cycle_return_label'] }}</p><p class="mt-0.5 text-[9px] text-zinc-400">every {{ $listingStory['return_interval_label'] }}</p></div>
                            <div class="border-r border-zinc-100 py-3 pr-3 dark:border-zinc-900"><p class="text-[9px] uppercase tracking-[.08em] text-zinc-400">Potential min</p><p class="mt-1 text-xs font-semibold">{{ currency_symbol() }}{{ number_format($listingStory['net_term_min_profit'],2) }}</p></div>
                            <div class="py-3 pl-3"><p class="text-[9px] uppercase tracking-[.08em] text-zinc-400">Potential max</p><p class="mt-1 text-xs font-semibold">{{ currency_symbol() }}{{ number_format($listingStory['net_term_max_profit'],2) }}</p></div>
                        </div>
                        <div class="mt-auto grid grid-cols-3 gap-3 border-t border-zinc-100 pt-4 dark:border-zinc-900">
                            <div><p class="text-[9px] text-zinc-400">Price</p><p class="mt-1 text-sm font-semibold">{{ currency_symbol() }}{{ number_format((float)$instrument->current_price,2) }}</p></div>
                            <div><p class="text-[9px] text-zinc-400">Move</p><p class="mt-1 text-sm font-semibold {{ $change >= 0 ? 'text-emerald-600' : 'text-red-600' }}">{{ $change >= 0 ? '+' : '' }}{{ number_format($change,2) }}%</p></div>
                            <div class="text-right"><p class="text-[9px] text-zinc-400">Minimum</p><p class="mt-1 text-sm font-semibold">{{ currency_symbol() }}{{ number_format((float)$instrument->minimum_investment,0) }}</p></div>
                        </div>
                    </a>
                @empty
                    <div class="col-span-full rounded-2xl border border-dashed border-zinc-300 bg-white p-10 text-center text-sm text-zinc-500 dark:border-zinc-800 dark:bg-zinc-950">No investment instruments match this view.</div>
                @endforelse
            </div>
        ...
        <aside class="space-y-4">
            <section class="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
                <p class="text-[10px] font-semibold uppercase tracking-[.14em] text-red-600">Market movement</p>
                <h2 class="mt-1 text-base font-semibold">Largest current moves</h2>
                <div class="mt-4 space-y-3">
                    ...
                    @empty
                        <p class="text-xs text-zinc-500">Movement data will appear as valuation history grows.</p>
                    @endforelse
                </div>
                ...
                <div class="mt-5 border-t border-zinc-200 pt-4 dark:border-zinc-800">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-[9px] font-semibold uppercase tracking-[.14em] text-zinc-400">Listings summary</p>
                            <p class="mt-1 text-sm font-semibold text-zinc-950 dark:text-white">{{ $summaryTotal }} active listings</p>
                        </div>
                        <a href="{{ route('investments.index') }}" class="text-[9px] font-semibold text-red-600 hover:text-red-700">View all</a>
                    </div>
                    ...
                    <div class="mt-3 flex items-center justify-between rounded-xl bg-zinc-50 px-3 py-2.5 dark:bg-zinc-900">
                        <span class="text-[9px] text-zinc-500">Underlying valuation</span>
