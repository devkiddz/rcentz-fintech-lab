<x-guest-layout>
    <x-slot name="title">{{ localize('ui.auth.sign_in', 'Sign in') }}</x-slot>
    ...
    <section class="ui-panel overflow-hidden">
        <div class="px-6 py-6 text-center">
            ...
            <h1 class="text-xl font-semibold tracking-tight text-foreground">{{ localize('ui.auth.sign_in', 'Sign in') }}</h1>
            <p class="mt-1 text-xs text-muted-foreground">{{ localize('ui.auth.access_account', 'Access your :site account', ['site' => site_name()]) }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-4 px-6 pb-6">
            @csrf
            <div>
                <label for="email" class="ui-label">{{ localize('ui.common.email', 'Email') }}</label>
            ...
            <div>
                <div class="mb-1.5 flex items-center justify-between">
                    <label for="password" class="ui-label mb-0">{{ localize('ui.common.password', 'Password') }}</label>
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-xs font-medium text-tesla-600 hover:text-tesla-700 dark:text-tesla-400">{{ localize('ui.common.forgot_password', 'Forgot password?') }}</a>
                    @endif
                </div>
                <input id="password" name="password" type="password" required autocomplete="current-password" class="ui-input @error('password') border-red-500 @enderror" placeholder="{{ localize('ui.auth.your_password', 'Your password') }}">
            ...
            <label class="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                {{ localize('ui.common.remember_me', 'Remember me') }}
            </label>

            <button type="submit" class="ui-button-primary w-full">{{ localize('ui.auth.sign_in', 'Sign in') }}</button>
        </form>

        <div class="border-t border-border bg-muted/30 px-6 py-4 text-center text-xs text-muted-foreground">
            {{ localize('ui.common.no_account', 'Don’t have an account?') }}
            <a href="{{ route('register') }}" class="font-semibold text-tesla-600 hover:text-tesla-700 dark:text-tesla-400">{{ localize('ui.nav.create_account', 'Create account') }}</a>
        </div>
    </section>

    <div class="my-5 flex items-center gap-3 text-xs text-muted-foreground">
        <div class="h-px flex-1 bg-border"></div><span>{{ localize('ui.common.or', 'or') }}</span><div class="h-px flex-1 bg-border"></div>
    </div>
    ...
    <a href="{{ route('google.redirect') }}" class="ui-button-secondary w-full">
        ...
        {{ localize('ui.common.continue_google', 'Continue with Google') }}
    </a>

    <p class="mt-5 flex items-center justify-center gap-1 text-center text-[11px] text-muted-foreground">
        <i data-lucide="shield-check" class="h-3.5 w-3.5"></i> {{ localize('ui.common.secure_account_access', 'Secure account access') }}
    </p>
</x-guest-layout>
