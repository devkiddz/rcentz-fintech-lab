...
<div class="ui-page max-w-[1500px]">
    <section class="ui-page-header">
        <div>
            <div class="flex flex-wrap items-center gap-2">
                <p class="ui-kicker">Private Base Asset</p>
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <form method="POST" action="{{ route('admin.investments.instruments.private.base-assets.status', $reference) }}">@csrf @method('PATCH')
    ...
    <section class="grid gap-3 sm:grid-cols-2 xl:grid-cols-7">
        ...
        <div class="ui-panel p-4"><p class="ui-kicker">Change Amount</p><p class="mt-2 text-xl font-semibold {{ $change === null ? 'text-muted-foreground' : ($change >= 0 ? 'text-emerald-600' : 'text-red-600') }}">{{ $change === null ? '—' : (($change >= 0 ? '+' : '').$prefix.number_format($change,2)) }}</p></div>
        <div class="ui-panel p-4"><p class="ui-kicker">Direction</p><p class="mt-2 text-xl font-semibold {{ $change === null ? 'text-muted-foreground' : ($change >= 0 ? 'text-emerald-600' : 'text-red-600') }}">{{ $change === null ? '—' : ($change > 0 ? '↑ UP' : ($change < 0 ? '↓ DOWN' : '→ FLAT')) }}</p></div>
        <div class="ui-panel p-4"><p class="ui-kicker">Base Unit</p><p class="mt-2 text-xl font-semibold">{{ $reference->reference_unit }}</p></div>
    ...
    <section class="mt-4 grid gap-4 xl:grid-cols-[.8fr_1.2fr]">
        <div class="ui-panel p-5">
            ...
            <form method="POST" action="{{ route('admin.investments.instruments.private.base-assets.price.update', $reference) }}" class="mt-5 space-y-4">
                @csrf @method('PATCH')
        ...
        <div class="ui-panel overflow-hidden">
            <div class="border-b border-border p-5">
                ...
                <h2 class="mt-1 text-lg font-semibold">Linked Investment Reserves</h2>
    ...
    <section class="ui-panel mt-4 overflow-hidden">
        <div class="border-b border-border p-5"><p class="ui-kicker">Valuation History</p><h2 class="mt-1 text-lg font-semibold">Base Price History</h2></div>
