<x-admin-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-xs text-muted-foreground mt-1">Edit the email template: {{ $template->name }}</p>
            </div>
    ...
    <div class="py-6">
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('admin.emails.templates.update', $template) }}" method="POST">
                @csrf
                @method('PATCH')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div class="space-y-3">
                                ...
                                <div>
                                    ...
                                    <select name="type" id="type" required
                                        ...
                                        <option value="purchase" {{ old('type', $template->type) === 'purchase' ? 'selected' : '' }}>Purchase</option>
                        ...
                        <div class="bg-card border border-border p-4 rounded-lg">
                            ...
                            <div>
                                ...
                                <textarea name="content" id="content" rows="12" required
                                          class="w-full px-3 py-2 border border-border rounded-lg focus:ring-2 focus:ring-black focus:border-black"
                                          placeholder="Write your email content here. You can use variables like @{{user_name}}, @{{user_email}}, etc.">{{ old('content', $template->content) }}</textarea>
                                @error('content')
                                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="bg-card border border-border p-4 rounded-lg">
                            ...
                            <div class="space-y-3">
                                <div>
                                    ...
                                    <input type="text" name="variables" id="variables" value="{{ old('variables', $template->variables) }}"
                                           class="w-full px-3 py-2 border border-border rounded-lg focus:ring-2 focus:ring-black focus:border-black"
                    ...
                    <div class="space-y-4">
                        ...
                        <!-- Template Info -->
                        <div class="bg-card border border-border p-4 rounded-lg">
                            <h3 class="text-base font-medium text-foreground dark:text-white mb-3">Template Info</h3>
                            
                            <div class="space-y-2 text-xs">
                                <div class="flex justify-between">
                                    <span class="text-muted-foreground dark:text-gray-300">Created:</span>
                                    <span class="text-foreground">{{ $template->created_at->format('M d, Y') }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-muted-foreground dark:text-gray-300">Last Updated:</span>
                                    <span class="text-foreground">{{ $template->updated_at->format('M d, Y') }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-muted-foreground dark:text-gray-300">Status:</span>
                        ...
                        <div class="bg-card border border-border p-4 rounded-lg">
                            <div class="space-y-2">
                                ...
                                <a href="{{ route('admin.emails.index') }}" 
                                   class="w-full inline-flex items-center justify-center px-4 py-2 bg-muted text-muted-foreground text-sm font-medium rounded-lg hover:bg-gray-200 transition-all duration-200">
                                    Cancel
                                </a>
                            </div>
                        </div>
