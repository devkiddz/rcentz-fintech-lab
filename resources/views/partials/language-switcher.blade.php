...
<details class="relative" data-language-switcher>
    ...
    <div class="absolute z-[90] mt-2 w-56 overflow-hidden rounded-xl border border-border bg-card p-1.5 shadow-2xl" style="inset-inline-end:0;">
        @foreach($languageOptions as $language)
            <form method="POST" action="{{ route('locale.update') }}">
                @csrf
                <input type="hidden" name="locale" value="{{ $language->code }}">
                <button type="submit" lang="{{ str_replace('_', '-', $language->code) }}" dir="{{ $language->direction }}" class="flex w-full items-center justify-between rounded-lg px-3 py-2 text-xs transition hover:bg-muted {{ $language->code === $activeLocale ? 'bg-muted font-semibold text-foreground' : 'text-muted-foreground' }}" style="text-align:start;">
