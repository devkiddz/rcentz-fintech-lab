    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-card rounded-xl p-6 shadow-sm border border-gray-100 mb-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-light text-black dark:text-white mb-1">Notifications</h1>
                    <p class="text-sm text-gray-600 dark:text-gray-300">{{ $unreadCount }} unread notifications</p>
                </div>
                @if($unreadCount > 0)
                    <button onclick="markAllAsRead()" 
                            class="px-4 py-2 bg-black dark:bg-white text-white dark:text-gray-900 text-sm font-medium rounded-lg hover:bg-gray-800 dark:hover:bg-gray-100 transition-colors duration-200 flex items-center">
        ...
        <div class="space-y-4">
                    ...
                    <div class="flex items-start space-x-4">
                        ...
                        <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0
                            ...
                            <i data-lucide="{{ $notification->icon }}" 
                               class="w-5 h-5 
                                      ...
                                      {{ $notification->color === 'indigo' ? 'text-indigo-600' : '' }}
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    <h3 class="text-sm font-medium text-black dark:text-white mb-1">
                                        {{ $notification->title }}
                                    </h3>
                                    <p class="text-sm text-gray-600 mb-2">
                                        {{ $notification->message }}
                                    </p>
                                    <div class="flex items-center space-x-4 text-xs text-gray-500 dark:text-gray-300">
                                ...
                                <div class="flex items-center space-x-2 ml-4">
                                    @if(!$notification->is_read)
                                        <button onclick="markAsRead({{ $notification->id }})" 
                                                class="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-muted rounded-lg transition-colors duration-200"
                                                title="Mark as read">
            ...
            @empty
                <div class="bg-card rounded-xl p-8 shadow-sm border border-gray-100 text-center">
                    ...
                    <p class="text-gray-600 dark:text-gray-300">You're all caught up! No notifications to show.</p>
                </div>
            @endforelse
        </div>
