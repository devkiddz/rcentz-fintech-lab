        ...
        <section class="mb-4 ui-panel overflow-hidden">
            <div class="flex flex-col gap-4 p-5 sm:p-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="flex min-w-0 items-start gap-4">
                    <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-border bg-muted/40 text-foreground"><i data-lucide="badge-check" class="h-5 w-5"></i></div>
                    <div class="min-w-0">
                        <p class="ui-kicker">Memberships</p>
                        @if($activeMemberships->isNotEmpty())
                            <h2 class="mt-1 text-lg font-semibold text-foreground">{{ $activeMemberships->count() }} active membership{{ $activeMemberships->count() === 1 ? '' : 's' }}</h2>
                        ...
                        @elseif($membershipStatuses->isNotEmpty())
                            <h2 class="mt-1 text-lg font-semibold text-foreground">Membership history available</h2>
                            <p class="mt-1 text-sm text-muted-foreground">Review your latest membership records and available plans.</p>
                        @else
                            <h2 class="mt-1 text-lg font-semibold text-foreground">Explore membership access</h2>
                            <p class="mt-1 text-sm text-muted-foreground">You do not currently have an active membership.</p>
                        @endif
                    </div>
                </div>
                <a href="{{ route('memberships.index') }}" class="ui-btn ui-btn-secondary"><i data-lucide="arrow-right" class="h-4 w-4"></i>Memberships</a>
            </div>
        </section>
