<x-user-layout>
    ...
    <div class="ui-page max-w-[1650px]">
        @php
            $counterparty = $conversation->participants
                ->first(fn ($p) => $p->user_id !== auth()->id() && $p->user)
                ?->user;
            $activeTitle = $workspace === 'messages'
                ? ($counterparty?->name ?: 'Private message')
        ...
        @if(session('error'))<div class="mb-3 rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-xs text-red-600">{{ session('error') }}</div>@endif

        <style>
            .rcentz-customer-messenger-shell { position: relative; min-height: 690px; }
            .rcentz-customer-info { display: none; }
            .rcentz-chat-menu { min-width: 14.5rem; width: max-content; max-width: calc(100vw - 2rem); white-space: nowrap; z-index: 70; }
            .rcentz-chat-menu button, .rcentz-chat-menu a { white-space: nowrap; }
            ...
            .rcentz-customer-inbox-backdrop { display: none; }
            @media (max-width: 1023px) {
                .rcentz-customer-inbox { position: absolute; inset: 0 auto 0 0; z-index: 55; display: flex; width: min(88vw, 340px); transform: translateX(-105%); transition: transform .2s ease; box-shadow: 18px 0 50px rgba(0,0,0,.25); }
                .rcentz-customer-messenger-shell[data-inbox-open="true"] .rcentz-customer-inbox { transform: translateX(0); }
                .rcentz-customer-messenger-shell[data-inbox-open="true"] .rcentz-customer-inbox-backdrop { position: absolute; inset: 0; z-index: 50; display: block; background: rgba(0,0,0,.45); }
            }
            @media (min-width: 1024px) {
                .rcentz-customer-messenger-shell { display: grid; height: calc(100vh - 145px); grid-template-columns: 310px minmax(0,1fr); }
                .rcentz-customer-inbox { display: flex; min-width: 0; }
            }
            @media (min-width: 1280px) {
                .rcentz-customer-messenger-shell { grid-template-columns: 310px minmax(0,1fr) 300px; }
                .rcentz-customer-info { display: flex; }
            }
        </style>
        <section id="customer-messenger-shell" class="rcentz-customer-messenger-shell overflow-hidden rounded-2xl border border-border bg-background shadow-sm" data-inbox-open="false">
            <button type="button" class="rcentz-customer-inbox-backdrop" data-customer-inbox-close aria-label="Close chats"></button>
            <aside class="rcentz-customer-inbox min-h-0 border-r border-border bg-muted/10 flex-col">
                <div class="border-b border-border p-4">
                    <div class="flex items-center justify-between gap-2">
                        <div><p class="ui-kicker">{{ $workspace === 'messages' ? 'Messages' : 'Support' }}</p><p class="mt-1 text-sm font-semibold">{{ $workspace === 'messages' ? 'Private conversations' : 'Support tickets' }}</p></div>
                        <div class="flex items-center gap-1"><a href="{{ route($routeBase.'.index') }}" class="flex h-9 w-9 items-center justify-center rounded-full border border-border hover:bg-muted" title="{{ $workspace === 'messages' ? 'Open messages' : 'New support ticket' }}"><i data-lucide="{{ $workspace === 'messages' ? 'messages-square' : 'square-pen' }}" class="h-4 w-4"></i></a><button type="button" class="flex h-9 w-9 items-center justify-center rounded-full border border-border hover:bg-muted lg:hidden" data-customer-inbox-close title="Close chats"><i data-lucide="x" class="h-4 w-4"></i></button></div>
                    </div>
                </div>
                <div class="min-h-0 flex-1 overflow-y-auto">
                    @foreach($conversationList as $chat)
                        @php
                            ...
                            $chatPeer = $chat->participants?->first(fn ($p) => $p->user_id !== auth()->id() && $p->user)?->user;
                            $chatTitle = $workspace === 'messages' ? ($chatPeer?->name ?: $chat->subject) : $chat->subject;
                        @endphp
                        <a href="{{ route($routeBase.'.show', $chat) }}" class="flex gap-3 border-b border-border/60 px-4 py-3 {{ $chat->id === $conversation->id ? 'bg-red-500/[.07]' : 'hover:bg-muted/35' }}">
                            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-muted text-xs font-bold">{{ strtoupper(mb_substr($chatTitle,0,1)) }}</div>
                            <div class="min-w-0 flex-1">
                                <div class="flex justify-between gap-2"><p class="truncate text-xs font-semibold">{{ $chatTitle }}</p><span class="shrink-0 text-[9px] text-muted-foreground">{{ $chat->last_message_at?->format('H:i') }}</span></div>
                                <div class="mt-1 flex items-center gap-2"><p class="min-w-0 flex-1 truncate text-[10px] text-muted-foreground">{{ $preview }}</p>@if(($chat->unread_count ?? 0)>0)<span class="rounded-full bg-red-500 px-1.5 py-0.5 text-[8px] font-bold text-white">{{ $chat->unread_count }}</span>@endif</div>
                            </div>
                        </a>
                    @endforeach
                </div>
            ...
            <main class="flex min-h-[690px] min-w-0 flex-col bg-muted/5 lg:min-h-0">
                <header class="flex items-center justify-between gap-3 border-b border-border bg-background/95 px-4 py-3 backdrop-blur sm:px-5">
                    <div class="flex min-w-0 items-center gap-3">
                        <button type="button" class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full hover:bg-muted" data-customer-inbox-toggle title="Toggle chats"><i data-lucide="panel-left" class="h-4 w-4"></i></button>
                        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-red-500/10 text-xs font-bold text-red-600">{{ strtoupper(mb_substr($activeTitle,0,1)) }}</div>
                        <div class="min-w-0">
                            <h1 class="truncate text-sm font-semibold">{{ $activeTitle }}</h1>
                            <p class="mt-0.5 truncate text-[10px] text-muted-foreground">{{ $workspace === 'messages' ? $conversation->subject.' · Private conversation' : $conversation->ticket_number.' · '.ucfirst($conversation->status) }}</p>
                        </div>
                    </div>

                    <details class="relative">
                        <summary class="flex h-9 w-9 cursor-pointer list-none items-center justify-center rounded-full hover:bg-muted"><i data-lucide="more-vertical" class="h-4 w-4"></i></summary>
                        <div class="rcentz-chat-menu absolute right-0 mt-2 rounded-xl border border-border bg-background p-1.5 shadow-2xl">
                            <form method="POST" action="{{ route($routeBase.'.archive', $conversation) }}">@csrf @method('PATCH')<input type="hidden" name="archived" value="{{ $participant?->archived_at ? 0 : 1 }}"><button class="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs hover:bg-muted"><i data-lucide="archive" class="h-4 w-4"></i>{{ $participant?->archived_at ? 'Move to chats' : 'Archive chat' }}</button></form>
                            <form method="POST" action="{{ route($routeBase.'.delete-for-me', $conversation) }}" onsubmit="return confirm('Delete this conversation for you? Existing messages will be hidden from your account.');">@csrf @method('DELETE')<button class="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs text-red-600 hover:bg-red-500/10"><i data-lucide="trash-2" class="h-4 w-4"></i>Delete chat for me</button></form>
                        </div>
                    </details>
                </header>
                    ...
                    @foreach($conversation->messages as $message)
                        @php
                            ...
                            $readByOther = $mine && $conversation->participants
                                ...
                                ->contains(fn($p) => (int)($p->last_read_message_id ?? 0) >= $message->id);
                                ...
                                <div class="rounded-2xl px-3.5 py-2.5 shadow-sm {{ $mine ? 'rounded-br-md bg-red-500/[.10] ring-1 ring-red-500/15' : 'rounded-bl-md bg-background ring-1 ring-border' }}">
                                    @if($replyVisible)
                                        <button type="button" onclick="document.getElementById('message-{{ $message->replyTo->id }}')?.scrollIntoView({behavior:'smooth',block:'center'})" class="mb-2 block w-full rounded-lg border-l-2 border-red-500 bg-muted/35 px-3 py-2 text-left">
                                            ...
                                            <span class="mt-0.5 block truncate text-[10px] text-muted-foreground">{{ $replyText }}</span>
                                        </button>
                                    @endif

                                        @if($images->isNotEmpty())
                                            <div class="mb-2 grid gap-1 overflow-hidden rounded-xl {{ $images->count() > 1 ? 'grid-cols-2' : 'grid-cols-1' }}">
                                                @foreach($images as $attachment)
                                                    <button type="button" class="overflow-hidden bg-muted" data-media-preview="{{ route($routeBase.'.attachments.preview', $attachment) }}" data-media-name="{{ $attachment->original_name }}">
                                                        <img src="{{ route($routeBase.'.attachments.preview', $attachment) }}" alt="{{ $attachment->original_name }}" class="h-44 w-full object-cover sm:h-52">
                                                    </button>
                                                @endforeach
                                            </div>
                                        @endif
                                        ...
                                        @if($message->body !== '')
                                            <p class="whitespace-pre-wrap break-words text-[13px] leading-5 text-foreground">{{ $message->body }}</p>
                                        @endif

                                        @if($files->isNotEmpty())
                                            <div class="mt-2 space-y-1.5">
                                                @foreach($files as $attachment)
                                                    <a href="{{ route($routeBase.'.attachments.download', $attachment) }}" class="flex items-center gap-2 rounded-lg bg-background/70 px-3 py-2 text-xs ring-1 ring-border/70 hover:bg-background">
                                                        <span class="flex h-8 w-8 items-center justify-center rounded-lg bg-muted"><i data-lucide="file-text" class="h-4 w-4"></i></span>
                                                        <span class="min-w-0 flex-1"><span class="block truncate font-medium">{{ $attachment->original_name }}</span><span class="text-[9px] text-muted-foreground">{{ number_format($attachment->size_bytes/1024,1) }} KB</span></span>
                                                        <i data-lucide="download" class="h-3.5 w-3.5 text-muted-foreground"></i>
                                                    </a>
                                                @endforeach
                                            </div>
                                        @endif
                                    <div class="mt-1.5 flex items-center justify-end gap-1 text-[8px] text-muted-foreground">
                                        <span>{{ $message->sent_at?->format('H:i') }}</span>
                                        @if($mine)
                                            <i data-lucide="{{ $readByOther ? 'check-check' : 'check' }}" class="h-3 w-3 {{ $readByOther ? 'text-sky-500' : '' }}"></i>
                                        @endif
                                    </div>
                                </div>
                                    ...
                                    <details class="rcentz-message-actions">
                                        ...
                                        <div class="rcentz-message-action-menu rcentz-chat-menu rounded-xl border border-border bg-background p-1.5 shadow-2xl">
                                            ...
                                            @if($mine && $message->body !== '')
                                                <button type="button" class="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs hover:bg-muted" data-edit-url="{{ route($routeBase.'.messages.edit', [$conversation,$message]) }}" data-edit-text="{{ $message->body }}"><i data-lucide="pencil" class="h-4 w-4"></i>Edit</button>
                                            @endif
                ...
                @if($conversation->status === 'closed')
                    <div class="border-t border-border bg-background px-5 py-4 text-center text-xs text-muted-foreground">This conversation is closed.</div>
                @else
                    <form method="POST" action="{{ route($routeBase.'.reply', $conversation) }}" enctype="multipart/form-data" class="border-t border-border bg-background p-3 sm:p-4" id="chat-composer" data-reply-action="{{ route($routeBase.'.reply', $conversation) }}">
                        @csrf
                        <input type="hidden" name="idempotency_key" value="{{ old('idempotency_key', (string) \Illuminate\Support\Str::uuid()) }}">
                        <input type="hidden" name="reply_to_message_id" id="reply-to-message-id" value="{{ old('reply_to_message_id') }}">

                        <div id="edit-preview" class="mb-2 hidden items-center gap-3 rounded-xl border-l-2 border-sky-500 bg-muted/35 px-3 py-2">
                            <div class="min-w-0 flex-1"><p class="text-[9px] font-semibold text-sky-600">Editing message</p><p class="truncate text-[10px] text-muted-foreground">Changes save silently in the conversation.</p></div>
                            <button type="button" id="clear-edit" class="flex h-7 w-7 items-center justify-center rounded-full hover:bg-muted"><i data-lucide="x" class="h-3.5 w-3.5"></i></button>
                        </div>

                        <div id="reply-preview" class="mb-2 hidden items-center gap-3 rounded-xl border-l-2 border-red-500 bg-muted/35 px-3 py-2">
                            <div class="min-w-0 flex-1"><p id="reply-preview-name" class="text-[9px] font-semibold text-red-600"></p><p id="reply-preview-text" class="truncate text-[10px] text-muted-foreground"></p></div>
                            <button type="button" id="clear-reply" class="flex h-7 w-7 items-center justify-center rounded-full hover:bg-muted"><i data-lucide="x" class="h-3.5 w-3.5"></i></button>
                        </div>

                        <div class="flex items-end gap-2">
                            <label class="flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-full hover:bg-muted" title="Attach files">
                                <input type="file" name="attachments[]" multiple accept=".jpg,.jpeg,.png,.webp,.pdf,.txt,.csv,.doc,.docx,.xls,.xlsx" class="sr-only" id="chat-files">
                                <i data-lucide="paperclip" class="h-5 w-5 text-muted-foreground"></i>
                            </label>
                            <div class="min-w-0 flex-1 rounded-2xl border border-border bg-muted/20 px-4 py-2">
                                <textarea name="message" rows="1" class="max-h-32 min-h-6 w-full resize-none bg-transparent text-sm outline-none placeholder:text-muted-foreground" placeholder="Message...">{{ old('message') }}</textarea>
                                <p id="chat-file-count" class="hidden pt-1 text-[9px] text-muted-foreground"></p>
                            </div>
                            <button class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-red-600 text-white hover:bg-red-700" title="Send"><i data-lucide="send" class="h-4 w-4"></i></button>
                        </div>
                    </form>
                @endif
            </main>

            <aside class="rcentz-customer-info min-h-0 border-l border-border bg-background flex-col">
                <div class="border-b border-border p-5 text-center">
                    <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-red-500/10 text-xl font-bold text-red-600">{{ strtoupper(mb_substr($activeTitle,0,1)) }}</div>
                    <h2 class="mt-3 text-sm font-semibold">{{ $activeTitle }}</h2>
                    <p class="mt-1 text-[10px] text-muted-foreground">{{ $workspace === 'messages' ? $conversation->subject : $conversation->ticket_number }}</p>
                </div>
                <div class="min-h-0 flex-1 space-y-5 overflow-y-auto p-4">
                    <section>
                        <div class="flex items-center justify-between"><p class="ui-kicker">Media</p><span class="text-[9px] text-muted-foreground">{{ $media['images']->count() }}</span></div>
                        @if($media['images']->isEmpty())
                            <p class="mt-2 text-[10px] text-muted-foreground">No shared images yet.</p>
                        @else
                            <div class="mt-3 grid grid-cols-3 gap-1.5">
                                @foreach($media['images'] as $attachment)
                                    <button type="button" class="aspect-square overflow-hidden rounded-lg bg-muted" data-media-preview="{{ route($routeBase.'.attachments.preview', $attachment) }}" data-media-name="{{ $attachment->original_name }}"><img src="{{ route($routeBase.'.attachments.preview', $attachment) }}" class="h-full w-full object-cover" alt="{{ $attachment->original_name }}"></button>
                                @endforeach
                            </div>
                        @endif
                    </section>
                    ...
                    <section class="border-t border-border pt-4">
                        <div class="flex items-center justify-between"><p class="ui-kicker">Files</p><span class="text-[9px] text-muted-foreground">{{ $media['files']->count() }}</span></div>
                        <div class="mt-2 space-y-2">
                            @forelse($media['files'] as $attachment)
                                <a href="{{ route($routeBase.'.attachments.download', $attachment) }}" class="flex items-center gap-2 rounded-xl border border-border p-2.5 hover:bg-muted/30"><span class="flex h-8 w-8 items-center justify-center rounded-lg bg-muted"><i data-lucide="file" class="h-4 w-4"></i></span><span class="min-w-0 flex-1"><span class="block truncate text-[10px] font-semibold">{{ $attachment->original_name }}</span><span class="text-[8px] text-muted-foreground">{{ number_format($attachment->size_bytes/1024,1) }} KB</span></span></a>
                            @empty
                                <p class="text-[10px] text-muted-foreground">No shared files yet.</p>
                            @endforelse
                        </div>
                    </section>

                    <section class="border-t border-border pt-4">
                        <p class="ui-kicker">Conversation info</p>
                        <div class="mt-3 space-y-2 text-[10px]">
                            <div class="flex justify-between gap-3"><span class="text-muted-foreground">Type</span><strong>{{ ucfirst(str_replace('_',' ',$conversation->type)) }}</strong></div>
                            <div class="flex justify-between gap-3"><span class="text-muted-foreground">Status</span><strong>{{ ucfirst($conversation->status) }}</strong></div>
                            @if($conversation->type === 'support_ticket')<div class="flex justify-between gap-3"><span class="text-muted-foreground">Priority</span><strong>{{ ucfirst($conversation->priority) }}</strong></div>@endif
                        </div>
                    </section>
                </div>
            </aside>
    ...
    <div id="media-lightbox" class="fixed inset-0 z-[90] hidden items-center justify-center bg-black/90 p-4">
        <button type="button" id="media-lightbox-close" class="absolute right-5 top-5 flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white"><i data-lucide="x" class="h-5 w-5"></i></button>
        <div class="max-h-full max-w-5xl text-center"><img id="media-lightbox-image" class="max-h-[82vh] max-w-full rounded-xl object-contain" alt=""><p id="media-lightbox-name" class="mt-3 text-xs text-white/70"></p></div>
    </div>

    <script>
        (() => {
            const shell = document.getElementById('customer-messenger-shell');
            document.querySelectorAll('[data-customer-inbox-toggle]').forEach((button) => button.addEventListener('click', () => {
                if (shell) shell.dataset.inboxOpen = shell.dataset.inboxOpen === 'true' ? 'false' : 'true';
            }));
            document.querySelectorAll('[data-customer-inbox-close]').forEach((button) => button.addEventListener('click', () => {
                if (shell) shell.dataset.inboxOpen = 'false';
            }));
            ...
            const resetEdit = (clearText = true) => {
                if (!form) return;
                form.action = form.dataset.replyAction;
            ...
            document.querySelectorAll('[data-edit-url]').forEach((button) => button.addEventListener('click', () => {
                resetEdit(false);
            ...
            document.getElementById('clear-edit')?.addEventListener('click', () => resetEdit(true));
            ...
            document.querySelectorAll('[data-reply-id]').forEach((button) => button.addEventListener('click', () => {
                resetEdit(true);
                replyInput.value = button.dataset.replyId;
                previewName.textContent = button.dataset.replyName || 'Message';
                previewText.textContent = button.dataset.replyText || 'Attachment';
                preview.classList.remove('hidden'); preview.classList.add('flex');
                document.querySelector('#chat-composer textarea[name="message"]')?.focus();
            }));
            document.getElementById('clear-reply')?.addEventListener('click', () => {
                replyInput.value = ''; preview.classList.add('hidden'); preview.classList.remove('flex');
            });

            const fileCount = document.getElementById('chat-file-count');
            files?.addEventListener('change', () => {
                if (!files.files.length) { fileCount.classList.add('hidden'); return; }
                fileCount.textContent = `${files.files.length} attachment${files.files.length === 1 ? '' : 's'} selected`;
            ...
            document.querySelectorAll('[data-media-preview]').forEach((button) => button.addEventListener('click', () => {
                ...
                lightboxImage.alt = button.dataset.mediaName || 'Shared image';
                lightboxName.textContent = button.dataset.mediaName || '';
                lightbox.classList.remove('hidden'); lightbox.classList.add('flex');
            }));
            const closeLightbox = () => { lightbox.classList.add('hidden'); lightbox.classList.remove('flex'); lightboxImage.src = ''; };
